...
def create_device(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Device.objects.update_or_create(name=form.cleaned_data["name"], type_id=form.cleaned_data["type"].pk)
...
def create_ssh_command(request, pk):
    if request.method == "POST":
        ...
        if form.is_valid():
            SSHCommand.objects.update_or_create(command=form.cleaned_data["command"], device_id=pk)
            return HttpResponseRedirect(
                reverse(f"io_ssh:detail", kwargs={"pk": pk})
            )
    else:
        form = SSHCommandForm()
    return render(
